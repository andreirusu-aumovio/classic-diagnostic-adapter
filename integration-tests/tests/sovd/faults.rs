...
fn assert_dtc_extended_data(sim_dtc: &DtcExtended, result_fault: ExtendedFault<VendorErrorCode>) {
    ...
    */
    ...
    for sim_snapshot in &sim_dtc.snapshots {
        ...
        let result_snapshot = result_snapshots.get(&sim_snapshot.record_number).unwrap_or_else(|| {
            panic!(
                "Expected snapshot record with key 0x{}",
        ...
        assert_eq!(
            ...
            "Snapshot record {}: expected {} identifier(s), got {}",
            sim_snapshot.record_number,
        ...
        assert_eq!(
            ...
            "Snapshot record {}: expected {} DID entry/entries, got {}",
            sim_snapshot.record_number,
            sim_snapshot.records.len(),
            result_snapshot.record.len()
        );
            ...
            .enumerate()
        {
            let expected_did = u16::from_str_radix(&test_record.did, 16)
                .unwrap_or_else(|_| panic!("Invalid hex DID in snapshot record {}, entry {idx}", sim_snapshot.record_number))
                as u64;
            let expected_data = u64::from_str_radix(&test_record.data, 16)
                .unwrap_or_else(|_| panic!("Invalid hex data in snapshot record {}, entry {idx}", sim_snapshot.record_number));

            // Extract Snapshot DID (as defined in test ODX)
            let result_did = result_entry
                .get("DTCSnapshotRecordDid")
                .and_then(|v| v.as_u64())
                .unwrap_or_else(|| panic!("Missing DTCSnapshotRecordDid in snapshot record {}, entry {idx}", sim_snapshot.record_number));
            // Extract Snapshot Data (as defined in test ODX)
            let result_did_data = result_entry
                .get("DTCSnapshotRecordDidData")
                .and_then(|v| v.as_u64())
                .unwrap_or_else(|| panic!("Missing DTCSnapshotRecordDidData in snapshot record {}, entry {idx}", sim_snapshot.record_number));

            assert_eq!(
                expected_did, result_did,
                "Snapshot record {}, entry {idx}: expected DID {expected_did:#06X}, got {result_did:#06X}",
                sim_snapshot.record_number
            );
            assert_eq!(
                expected_data, result_did_data,
                "Snapshot record {}, entry {idx}: expected data {expected_data:#010X}, got {result_did_data:#010X}",
                sim_snapshot.record_number
            );
        }
    }
